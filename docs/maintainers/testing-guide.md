...
- `coverage.xml` — optional generic XML (generated by `task test` in root)
...
Coverage exclusions are applied in `Taskfile.yml` when generating reports (tests, mocks, app shell).
...
```bash
task test    # SPM tests + local coverage reports
```
...
3. **"Coverage is lower than expected"**
   - Check exclusion patterns in `Taskfile.yml`
   - Only `MagSafeGuardLib` is measured for SPM coverage
